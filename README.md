...
Application uses a number of digital I/O pins, analog input pins and 3.3 V and 5 V power supply. Schematic can be seen here [SCHEMATIC](https://github.com/amwellius/DE2_Project_2021-22/blob/main/schematic.pdf). 
...
Usage of relay(s) is optional. **C code** has implemented section which toggles the relay module by pressing **button** connected to Arduio UNO. It can be connected to pump or solenoid to enable watering plants or other uses of the water. **A capacitor** inserted in cicruit (as RL low pass filter) helps to stabilize *overoscillations* generated by the button. By using hardware filter we were able to implement button handling with interrupt.
